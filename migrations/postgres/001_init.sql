-- Trenches week 1, Postgres. Mirror of migrations/sqlite/001_init.sql.
--
-- Identical to the SQLite schema except for the identity column. Timestamps
-- stay ISO-8601 UTC TEXT and token amounts stay exact-integer TEXT on BOTH
-- engines -- not because Postgres lacks better types, but because using them
-- would put type coercion in the DAL and make the connection-string swap a
-- lie. Revisit only if a query actually gets slow.
--
